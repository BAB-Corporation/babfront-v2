...
class BotfrontTrackerStore(TrackerStore):
    def __init__(self, domain, host, **kwargs):
        ...
        self.max_events = kwargs.get("max_events", 50)
        self.max_event_history = kwargs.get("max_event_history", 100)
        self.trackers = {}
        self.test_trackers = {}
        self.trackers_info = (
            {}
    ...
    def _convert_tracker(self, sender_id, tracker):
        if self.domain:
            ...
            return DialogueStateTracker.from_dict(
                sender_id, tracker["events"], self.domain.slots, max_event_history
            )
        else:
            logger.warning(
    ...
    def _update_tracker(self, sender_id, remote_tracker):
        ...
        logger.warning("Old tracker when update")
        logger.warning(len(old_tracker))
        logger.warning("Remote tracker when update")
        logger.warning(len(remote_tracker))
